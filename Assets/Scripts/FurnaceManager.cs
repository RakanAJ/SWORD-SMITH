...
public class FurnaceManager : MonoBehaviour
{
    ...
    private void Smelt()
    {
        ...
        var smelted = _smeltedObject;
        StartCoroutine(nameof(InitializeInteractable), smelted);
        _smeltedObject = null;
    ...
    IEnumerator InitializeInteractable(GameObject instantiatedObject)
    {
        yield return null; // Wait for one frame
        var interactable = instantiatedObject.GetComponent<DistanceHandGrabInteractable>();
        var grabble = instantiatedObject.GetComponentInChildren<DistanceHandGrabInteractable>();
        if (interactable != null)
        {
            interactable.enabled = false;
            interactable.enabled = true; // Ensure it's enabled
        }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.GetComponent<Smelt>())
        {
            ...
            if (smeltObject && smeltObject.canBeSmelted)
            {
                ...
                {
                    var grabbable = other.GetComponent<DistanceGrabInteractable>();
